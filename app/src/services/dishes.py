...
class DishesService(BaseService):
    ...
    async def get_by_submenu(self, menu_id: UUID, submenu_id: UUID) -> Sequence[models.Dishes | dict]:
        return await self.repo.get_by_submenu(menu_id, submenu_id)

    async def get(self, menu_id: UUID, submenu_id: UUID, dish_id: UUID) -> models.Dishes | dict:
        return await self.repo.get_by_ids_path(menu_id, submenu_id, dish_id)

    async def create(self, menu_id: UUID, submenu_id: UUID, dish: schemas.CreateDish) -> models.Dishes:
    ...
    async def update(self, menu_id: UUID, submenu_id: UUID, dish_id: UUID,
                     submenu: schemas.UpdateDish) -> models.Dishes:
